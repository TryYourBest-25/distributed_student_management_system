...
{
    public async Task<ReadRecordResult<StudentRecord>> ReadFromFileAsync(IFormFile file,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (await csvReader.ReadAsync())
            {
                ...
                {
                    logger.LogError(e, "Error reading record at line {LineNumber}",
                        csvReader.Context.Reader?.CurrentIndex);
                    current++;
                }
            }
