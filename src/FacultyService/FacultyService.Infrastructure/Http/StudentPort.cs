...
public class StudentPort(
    ...
    IMultiTenantStore<AppTenantInfo> tenantStore) : IStudentPort
{
    public async Task<Result<Student>> CreateStudentAsync(Student student)
    {
        ...
        var tenant = await tenantStore.TryGetAsync(student.FacultyCode);
        if (tenant is null)
        {
            return Result.Fail<Student>($"Không tìm thấy server của khoa {student.FacultyCode}");
        }
        ...
        // Parse response data
        var studentResponse = await response.Content.ReadFromJsonAsync<StudentBasicResponse>();
    ...
    public async Task<Result<int>> DeleteStudentAsync(StudentCode studentCode, FacultyCode facultyCode)
    {
        // Lookup tenant information using student's faculty code
        var tenant = await tenantStore.TryGetAsync(studentCode.FacultyCode);
        if (tenant is null)
        {
