...
public partial record StudentCode
{
    ...
    public StudentCode(string value)
    {
        ...
        {
            throw new BadInputException($"Mã sinh viên không hợp lệ. Mã sinh viên phải có định dạng NxxDCxxddd, trong đó x là chữ cái và d là chữ số.");
        }
    ...
    public static StudentCode Of(ClassCode code, int studentNumber) => new StudentCode($"N{code.AcademicYear}DC{code.MajorCode}{studentNumber:D3}");
