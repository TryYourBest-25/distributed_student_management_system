...
CREATE DATABASE IF NOT EXISTS faculty_it_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE faculty_it_db;

-- Bảng Lớp học (chỉ chứa lớp của khoa CNTT)
CREATE TABLE class (
    ...
    faculty_code CHAR(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Mã khoa (luôn là CNTT ở DB này)',
    ...
    UNIQUE (class_name),
    -- Kiểm tra định dạng mã lớp khoa CNTT mới dựa trên ví dụ: D<YY>CQ<Major><NN>
    -- Major có thể là CN, CP, IS, MT. Chấp nhận d và cq.
    CONSTRAINT chk_class_code_format_it CHECK (class_code REGEXP '^[Dd][0-9]{2}[Cc][Qq](CN|CP|IS|MT)[0-9]{2}$'),
    -- Kiểm tra mã khoa phải là CNTT
    CONSTRAINT chk_faculty_code_it CHECK (faculty_code = 'CNTT')
) COMMENT='Bảng chứa thông tin các lớp thuộc khoa CNTT';

-- Bảng Sinh viên (chỉ chứa sinh viên khoa CNTT)
CREATE TABLE student (
    ...
    -- Kiểm tra định dạng mã sinh viên khoa CNTT: N<YY>DCCN<NNN>
    CONSTRAINT chk_student_code_format_it CHECK (student_code REGEXP '^N[0-9]{2}DCCN[0-9]{3}$')
) COMMENT='Bảng chứa thông tin sinh viên khoa CNTT';

-- Bảng Lớp tín chỉ (chỉ chứa lớp tín chỉ do khoa CNTT quản lý)
CREATE TABLE credit_class (
    ...
    faculty_code CHAR(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Mã khoa quản lý (luôn là CNTT ở DB này)',
    ...
    -- CONSTRAINT fk_credit_class_course FOREIGN KEY (course_code) REFERENCES course_replica(course_code), -- Tham chiếu bản sao môn học
    -- CONSTRAINT fk_credit_class_lecturer FOREIGN KEY (lecturer_code) REFERENCES lecturer_replica(lecturer_code), -- Tham chiếu bản sao giảng viên
    CONSTRAINT chk_credit_class_faculty_it CHECK (faculty_code = 'CNTT'),
...
) COMMENT='Bảng chứa thông tin các lớp tín chỉ do khoa CNTT quản lý';

-- Bảng Đăng ký (chỉ chứa đăng ký của sinh viên khoa CNTT)
...
) COMMENT='Bảng chứa thông tin đăng ký lớp tín chỉ của sinh viên khoa CNTT';
